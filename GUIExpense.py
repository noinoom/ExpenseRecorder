...
ws = GUI.winfo_screenwidth() #screen width เช็คขนาดหน้าจอ
...
def Save(event=None):
	...
	try:
		...
		transactionid = stamp.strftime('A%Y%m%d%H%M%f')
		dt = days[today] + '-' + dt
...
main_icon = PhotoImage(file='main.png')
...
########Right Click Menu#########
def EditRecord():
	POPUP = Toplevel() # คล้ายๆกับ Tk() จะสร้างตารางใหม่ เพราะ Tk จะมีการประกาศใช้แค่ครั้งเดียว
	...
	def Edit():
		...
		alltransaction[str(transactionid)] = newdata  #แปลง transaction ให้เป็นข้อมูลใหม่
		UpdateCSV()
		update_table()
